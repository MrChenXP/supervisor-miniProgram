...
Page({
  ...
  // 去 协商意见 新增 处理|验收  
  toXs(){
    wx.navigateTo({ url:"/pages/zggz/xsyj/xsyj"})
  },
  toAdd(e){
    let id = e.currentTarget.dataset.id
    if (id !== 'add') { // 不是新增就是审核
      wx.navigateTo({ url:"/pages/zggz/zggz-add/zggz-add?ZGXSID="+id})
    } else {
      wx.navigateTo({ url:"/pages/zggz/zggz-add/zggz-add"})
    }
  },
  toZgxs(e) {
    ...
    // 不是详情那就是-处理验收
    if (status !== undefined) {
      wx.navigateTo({url:`/pages/zggz/zggz-preview/zggz-preview?ZGXSID=${id}&SF=${status}`})
    } else {
      wx.navigateTo({url:"/pages/zggz/zggz-preview/zggz-preview?ZGXSID="+id})
    }
  },
  // 删除
  deleteAction() {
